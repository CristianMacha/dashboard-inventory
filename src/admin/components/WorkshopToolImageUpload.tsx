...
import { uploadWorkshopToolImageAction, deleteWorkshopToolImageAction } from "@/admin/actions/upload-workshop-tool-image.action";
import { workshopToolKeys } from "@/admin/queryKeys";
import { getCloudinaryUrl } from "@/lib/cloudinary";
import { getErrorMessage } from "@/api/apiClient";

interface WorkshopToolImageUploadProps {
  toolId: string;
...
export const WorkshopToolImageUpload = ({
  toolId,
  imagePublicId: initialImagePublicId,
  page,
  limit,
}: WorkshopToolImageUploadProps) => {
  ...
  const uploadMutation = useMutation({
    mutationFn: (file: File) => uploadWorkshopToolImageAction(toolId, file),
    onSuccess: () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: () => deleteWorkshopToolImageAction(toolId),
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: workshopToolKeys.list({ page, limit }) });
