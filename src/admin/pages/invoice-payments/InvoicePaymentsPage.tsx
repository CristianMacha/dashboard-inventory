...
} from "@/admin/actions/get-invoice-payments.action";
import { invoicePaymentKeys } from "@/admin/queryKeys";
...
const invoicePaymentColumns: ColumnDef<InvoicePaymentResponse>[] = [
  {
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <Button variant="link" size="sm" className="h-auto p-0 font-medium" asChild>
        <Link to={`/purchase-invoices/${row.original.invoiceId}`}>
...
export const InvoicePaymentsPage = () => {
  ...
  const { data, isLoading, isError, refetch } = useQuery({
    queryKey: invoicePaymentKeys.list(queryParams),
    queryFn: () => getInvoicePaymentsAction(queryParams),
  });
      ...
      <div className="flex items-center justify-between">
        <Breadcrumb>
          <BreadcrumbList>
            ...
            <BreadcrumbSeparator />
            <BreadcrumbItem>
              <BreadcrumbPage>Invoice Payments</BreadcrumbPage>
            </BreadcrumbItem>
          </BreadcrumbList>
        </Breadcrumb>
      </div>
